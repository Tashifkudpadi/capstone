...
import { Brain, TrendingUp, Workflow, GitBranch, Cloud, Shield, Database, Heart, Network, Users } from "lucide-react"
import { useScrollAnimation } from "@/hooks/use-scroll-animation"

const themes = [
  { icon: Brain, title: "AI-Powered Digital Enterprises" },
  { icon: TrendingUp, title: "Strategic Digital Transformation & Value Realization" },
...
export function KeyThemes() {
  ...
  return (
    <section ref={ref} id="themes" className="py-20 lg:py-32 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <h2 className="text-3xl lg:text-4xl font-bold text-gray-900">Strategic focus areas for CIO leadership</h2>
        </div>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4">
          {themes.map((theme, index) => (
            ...
            >
              ...
              <h3 className="text-sm font-semibold text-gray-900 leading-snug pr-8">{theme.title}</h3>
            </div>
          ))}
        </div>
      </div>
